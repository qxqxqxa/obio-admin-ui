    ...
    <el-table v-loading="state.dataListLoading" :data="state.dataList" row-key="id" border style="width: 100%">
      ...
      <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="170">
        <template v-slot="scope">
          <el-button v-if="scope.row.menuType === 0" type="primary" link @click="addHandle(scope.row)">{{ $t("add") }}</el-button>
...
import AddOrUpdate from "./categories-add-or-update.vue";

const view = reactive({
  getDataListURL: "/cms/categories/list",
  deleteURL: "/cms/categories"
});
